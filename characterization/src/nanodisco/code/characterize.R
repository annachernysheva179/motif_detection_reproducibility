...
path_models <- "benchmark/src/nanodisco/models/"
path_script <- "benchmark/src/nanodisco/code/"
...
# nb_threads <- 10
# base_name <- "meme_Filmore_peaks_uBH_0.001_peak_dist_2_min_cov_20_min_dist_20"
# path_diff_data <- "data/samples/Filmore/diff/Filmore_difference_FN.RDS"
# path_output <- "benchmark/characterize/meme/meme_Filmore_peaks_uBH_0.001_peak_dist_2_min_cov_20_min_dist_20"
# list_motif <- str_split("AANNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNGTC,CGCNNNNNNNNNNNNNNNNNNNNNNACC,CGCNNNNNNNNNNNNNNNACC,ATTNNNNNNNNNNNNNNNGAT,ACANNNNNNNNNNNNNNNAAA,CCANNNNNNNNNNCCA,ACANNNNNNNNNNCAA", ",", simplify = TRUE)[1, ]
# list_model <- "nn"
# genome <- "data/samples/Filmore/ref_genome/Filmore.fasta"
# list_contig <- NA
...
stifle <- foreach(model_name = list_model) %do% {
  ...
  write.table(classification_results, file = paste0(path_output, "all_motifs_", model_name, "_model_classification.tsv"), quote = FALSE, sep = "\t", row.names = FALSE, col.names = TRUE)
