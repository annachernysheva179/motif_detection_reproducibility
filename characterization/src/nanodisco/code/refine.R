...
# MOD: refine potential motifs
print_message("Refine potential motifs")

newly_discovered_motifs <- foreach(potential_motif = list_motif) %do% {
    tmp_newly_discovered_motifs <- auto.confirm.motif(list_motif, potential_motif, difference_data, genome, nb_threads, seq_params, iupac_nc, path_output, base_name, score_threshold = 0.0)
    print(tmp_newly_discovered_motifs)
    return(tmp_newly_discovered_motifs)
}

json_string <- jsonlite::toJSON(newly_discovered_motifs, pretty = TRUE)
output_file <- file.path(path_output, paste0(base_name, "confirmed_motif.json"))
writeLines(json_string, con = output_file)
