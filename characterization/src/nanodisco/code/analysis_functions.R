load.libraries.all <- function() {
    ...
    library(e1071)
    library(magrittr) # MOD: check the intermeidate data frames
}
...
extract.motifs.signature <- function(methylation_signal, genome, motif_summary, iupac_nc, left_signal, right_signal, error_margin, expected_signal_left, expected_signal_right, signal_margin, filter_iso, min_cov, nbCPU) {
    ...
    expected_signal <- na.omit(expected_signal) # MOD: Remove NA for skipping unvalid motifs
...
draw.local.signal <- function(base_name, final_stat_data, motif_summary, param_local_signal, genome) {
    ...
    if (length(unique(sample_data$position)) < 50) {
        return(NULL)
    } # Tweak limits; Stop if less than 50 data points overall
...
execute.meme <- function(input_fasta_name, meme_output, mod, nbmotif, min_motif_size, max_motif_size, nbCPU) {
    cmd_meme <- paste("meme", input_fasta_name, "-dna -oc", meme_output, "-mod", mod, "-nmotifs", nbmotif, "-minw", min_motif_size, "-maxw", max_motif_size, "-p", nbCPU, "-maxsize 1000000")
...
read.meme.output <- function(meme_output_xml) {
    ...
    motifs_length <- motifs_info %>%
        xml_attr("width") %>%
        as.integer()
    motifs_nbhits <- motifs_info %>%
        xml_attr("sites") %>%
        as.integer()
    motifs_evalue <- motifs_info %>%
        xml_attr("e_value") %>%
        as.numeric()
    motifs_llr <- motifs_info %>%
        xml_attr("llr") %>%
        as.integer()
    ...
    motifs_probs_results <- foreach(idx = seq(1, length(motifs_prob_detail)), .combine = rbind) %do% { # Not needed but easier to read
        letter_id <- motifs_prob_detail[idx] %>%
            xml_find_all("descendant::value") %>%
            xml_attr("letter_id")
        prob <- motifs_prob_detail[idx] %>%
            xml_find_all("descendant::value") %>%
            xml_contents() %>%
            xml_text() %>%
            as.numeric() # as.integer() if scores
    ...
    motifs_scores_results <- foreach(idx = seq(1, length(motifs_scores_detail)), .combine = rbind) %do% { # Not needed but easier to read
        letter_id <- motifs_scores_detail[idx] %>%
            xml_find_all("descendant::value") %>%
            xml_attr("letter_id")
        prob <- motifs_scores_detail[idx] %>%
            xml_find_all("descendant::value") %>%
            xml_contents() %>%
            xml_text() %>%
            as.integer() # as.numeric() if scores
    ...
    # MOD: remove this line, so that the dataframe contains no NA values for motif name. both two df haven't been assigned any levels.
...
refine.motif <- function(potential_motif, discovered_motifs, final_stat_data, genome, nbCPU, seq_params, iupac_nc, path_output, graph_subtitle, automated = TRUE) {
    ...
    if (!is.null(modification_at_motifs)) {
        ...
        write.table(mutated_motif_score, file = paste0(path_output, "/", "mutated_motif_score_", compound_name, ".tsv"), sep = "\t", quote = FALSE, row.names = FALSE, col.names = TRUE)
        write.table(modification_at_motifs, file = paste0(path_output, "/", "modification_at_motifs_", compound_name, ".tsv"), sep = "\t", quote = FALSE, row.names = FALSE, col.names = TRUE)
...
auto.confirm.motif <- function(discovered_motifs, potential_motif, final_stat_data, genome, nbCPU, seq_params, iupac_nc, meme_output, graph_subtitle, score_threshold = 2) {
    ...
    if (!is.null(refinement_data)) {
        ...
        # MOD: change nrow(res) > 3 to nrow(res) > 2
        if (nrow(res) > 2) {
...
user.confirm.motif <- function(discovered_motifs, potential_motif, final_stat_data, genome, nbCPU, seq_params, iupac_nc, meme_output, graph_subtitle) {
    ...
    while (has_responded0 == 0) {
        ...
        if (response == "Y") {
            ...
            # Do nothing and return original discovered_motifs
            has_responded0 <- 1
        } else {
            has_responded0 <- 0
        }
        ...
        tryCatch(
            {
                ...
                # MOD: write the result from tag.motifs to a file
                write.table(tagged_final_stat_data, file = paste0(path_output, "Motifs_occurrences_scores_", motif, ".tsv"), sep = "\t", quote = FALSE, row.names = FALSE, col.names = TRUE)
            ...
            error = function(e) {
                print(paste0("Unexpected error for ", motif))
                return(NULL)
            }
        )
    }
...
draw.tsne.signature.heatmap.detailed <- function(motifs_signature, motif_summary, tsne_data, genome, selected_pos, hc_pos, base_name, nbCPU, clustering_method = "kmeans") {
    ...
    list_plots <- foreach(idx_selected_motif = seq(1, nrow(motif_summary)), .inorder = TRUE) %do% {
        ...
        ordered_id <- unlist(sapply(list_hc, function(x) {
            return(x$label[x$order])
        }))
...
classify.detected.motifs <- function(methylation_signal, strain_id, motif_center_summary, model, genome, min_cov, keepIsolated, iupac_nc, nbCPU) {
    ...
    classification_results <- foreach(idx_motif = seq(1, nrow(motif_center_summary)), .combine = rbind) %do% {
        tryCatch(
            ...
            error = function(e) {
                message(paste0("Error in classifing motif ", motif_center_summary$motif[idx_motif]))
...
new.summarize.feature.ROC <- function(detailed_parameters, stat_data, ROC_parameters, ROC_classes, list_exception_motifs) {
    ...
    ROC_res <- stat_data %>%
        group_by(dir, strand) %>%
        do(evaluate.feature.ROC(., start = ROC_parameters$primary$start, end = ROC_parameters$primary$end, threshold = threshold, ROC_classes = ROC_classes))
...
prepare.parameters.ROC <- function(smoothing_win_sizes, peak_filtering_radii, peak_widths, win_sizes, offsets, start_points, coverages = NA, thresholds = NA) {
    if (is.na(min(coverages))) {
        coverages <- 4
    } # Min produce NA if in list
    if (is.na(min(thresholds))) {
        thresholds <- seq(0, 32, 0.1)
    } # 0.1 32
    parameters <- expand.grid(list(
        smoothing_win_size = smoothing_win_sizes, peak_filtering_radius = peak_filtering_radii,
...
compute.AUC <- function(ROC_data) {
    ...
    if ("dataset_name" %in% colnames(ROC_data)) {
        ...
        res <- ROC_data %>%
            group_by(dataset_name, min_cov, smoothing_win_size, motif) %>%
            do(myAUC(.))
    } else {
        # res <- ddply(ROC_data, .(min_cov,smoothing_win_size,motif), myAUC, .parallel=FALSE)
        res <- ROC_data %>%
            group_by(min_cov, smoothing_win_size, motif) %>%
            do(myAUC(.))
    }
